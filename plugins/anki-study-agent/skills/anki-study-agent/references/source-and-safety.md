...
The current implementation supports local files or directories containing text, Markdown, source code, HTML, and subtitle text. It also supports anonymous static HTTPS webpage snapshots and YouTube subtitle snapshots entered through the trusted local URL window. Text-layer PDF is supported only when `system.get_capabilities` reports `sourceAdapters.pdfTextLayer.available=true`; its B-tier representation preserves page evidence and declares omitted content. A registered source is represented by an opaque grant-backed reference; the model receives neither arbitrary filesystem authority nor a raw URL.

Local video decoding, full YouTube video/audio ingestion, arbitrary public video, scanned PDF/OCR, Office extraction, audio or podcast transcription, dynamic/login webpage capture, and resumable source acquisition are not implemented by the current public tool set. Do not claim PDF support merely because Codex can see an attachment: check the adapter capability first, and stop when the inspection reports an empty text layer or sandbox blocker. Require a future capability/tool or ask the user for a supported extracted-text form.
