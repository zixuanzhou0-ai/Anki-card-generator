...
5. `system.request_source_grant` for a local file or directory. For a static webpage, explicit podcast response, or YouTube subtitle source, call `system.request_network_grant` only with a stable `grantRequestId`, `kind=trusted_entry`, and the source type; the user enters the complete address in the trusted local window, never in tool arguments. Current `web` and `podcast` registration make an anonymous bounded HTTPS response snapshot; a podcast grant does not follow feed enclosures automatically. Current `public_video` supports YouTube subtitles only. Treat `other`, full media acquisition, enclosure following, dynamic/login pages, and unsupported parsers as explicit blockers.
