...
def _require_windows() -> None:
    if os.name != "nt":
        raise WindowsSandboxAclError(
            "WINDOWS_ACL_UNAVAILABLE", "Windows sandbox ACLs are unavailable"
        )
...
def _stable_existing_path(raw: str | Path) -> Path:
    ...
    if not path.is_absolute():
        raise WindowsSandboxAclError(
            "WINDOWS_ACL_PATH_RELATIVE", "ACL target must be absolute"
        )
    ...
    for part in absolute.parts[1:]:
        ...
        if current.is_symlink() or attributes & reparse_flag:
            raise WindowsSandboxAclError(
                "WINDOWS_ACL_REPARSE_BLOCKED", "ACL target contains a reparse point"
            )
...
def _sid_from_text(advapi32: ctypes.WinDLL, value: str) -> ctypes.c_void_p:
    ...
    if not advapi32.ConvertStringSidToSidW(value, ctypes.byref(sid)):
        raise WindowsSandboxAclError(
            "WINDOWS_SID_INVALID", f"Invalid sandbox SID: {ctypes.get_last_error()}"
        )
    return sid


def _sid_to_text(
    kernel32: ctypes.WinDLL, advapi32: ctypes.WinDLL, sid: ctypes.c_void_p
) -> str:
    text = wintypes.LPWSTR()
    if not advapi32.ConvertSidToStringSidW(sid, ctypes.byref(text)):
        raise WindowsSandboxAclError(
            "WINDOWS_SID_INVALID", f"Could not encode SID: {ctypes.get_last_error()}"
        )
    try:
...
def current_user_sid() -> str:
    ...
    try:
        ...
        if (
            ...
            raise WindowsSandboxAclError(
                "WINDOWS_TOKEN_UNAVAILABLE", "Could not size token user information"
            )
...
def _runtime_appcontainer_name(package_id: str) -> str:
    suffix = hashlib.sha256(
        f"study.runtime-appcontainer.v1:{package_id}".encode("utf-8")
    ).hexdigest()[:32]
...
def ensure_runtime_sandbox_profile(
    package_id: str = "anki-study-managed-runtime",
) -> str:
...
def task_sandbox_sid(task_id: str) -> str:
    try:
        ...
        raise WindowsSandboxAclError(
            "WINDOWS_TASK_ID_INVALID", "Task ID must be a canonical UUID"
        ) from error
...
def apply_exact_dacl(
    ...
    *,
    ...
    kernel32, advapi32 = _libraries()
    normalized = sorted(
        {(str(sid), int(mask)) for sid, mask in grants},
        key=lambda item: item[0].encode("utf-8"),
    )
    if not normalized:
        raise WindowsSandboxAclError(
            "WINDOWS_ACL_EMPTY", "ACL grants must not be empty"
        )
    ...
    try:
        ...
        inheritance = (
            OBJECT_INHERIT_ACE | CONTAINER_INHERIT_ACE if inherit_to_children else 0
        )
        for index, (sid_text, access_mask) in enumerate(normalized):
            ...
            entries[index].Trustee = TRUSTEE_W(
                None, 0, TRUSTEE_IS_SID, TRUSTEE_IS_UNKNOWN, sid
            )
        ...
        if status != 0:
            raise WindowsSandboxAclError(
                "WINDOWS_ACL_BUILD_FAILED", f"SetEntriesInAclW failed: {status}"
            )
        ...
        if status != 0:
            raise WindowsSandboxAclError(
                "WINDOWS_ACL_APPLY_FAILED", f"SetNamedSecurityInfoW failed: {status}"
            )
    finally:
        if new_acl:
...
def read_dacl(
    path: str | Path,
    *,
    reject_unsupported_flags: bool = False,
    ...
    if status != 0 or not dacl:
        raise WindowsSandboxAclError(
            "WINDOWS_ACL_READ_FAILED", f"GetNamedSecurityInfoW failed: {status}"
        )
    try:
        ...
        for index in range(header.AceCount):
            ...
            if not advapi32.GetAce(dacl, index, ctypes.byref(ace_pointer)):
                raise WindowsSandboxAclError(
                    "WINDOWS_ACL_READ_FAILED",
                    f"GetAce failed: {ctypes.get_last_error()}",
                )
            ace = ctypes.cast(ace_pointer, ctypes.POINTER(ACCESS_ALLOWED_ACE)).contents
            if ace.Header.AceType != ACCESS_ALLOWED_ACE_TYPE:
                raise WindowsSandboxAclError(
                    "WINDOWS_ACL_UNEXPECTED_ACE",
                    "Sandbox DACL contains a non-allow ACE",
                )
            if reject_unsupported_flags and int(ace.Header.AceFlags) & ~(
                OBJECT_INHERIT_ACE | CONTAINER_INHERIT_ACE
            ):
                raise WindowsSandboxAclError(
                    "WINDOWS_ACL_UNEXPECTED_FLAGS",
                    "Sandbox DACL contains inherited or unsupported ACE flags",
                )
            if skip_inherit_only and int(ace.Header.AceFlags) & INHERIT_ONLY_ACE:
                continue
            ...
            entries.append(
                DaclEntry(
                    ...
                    inheritance_flags=int(ace.Header.AceFlags)
                    & (OBJECT_INHERIT_ACE | CONTAINER_INHERIT_ACE),
                )
            )
...
def read_security_descriptor_identity(path: str | Path) -> SecurityDescriptorIdentity:
    target = _stable_existing_path(path)
    kernel32, advapi32 = _libraries()
    ...
    if status != 0 or not security_descriptor or not owner:
        raise WindowsSandboxAclError(
            "WINDOWS_SECURITY_DESCRIPTOR_READ_FAILED",
            f"GetNamedSecurityInfoW failed: {status}",
        )
    try:
        ...
        return SecurityDescriptorIdentity(
            owner_sid=_sid_to_text(kernel32, advapi32, owner_from_descriptor),
...
def runtime_tree_grants(
    sandbox_sid: str, *, user_sid: str | None = None
) -> tuple[tuple[str, int], ...]:
    return (
...
def task_workspace_grants(
    task_sid: str, *, user_sid: str | None = None
) -> tuple[tuple[str, int], ...]:
    return (
        (user_sid or current_user_sid(), FILE_FULL_CONTROL),
        (SYSTEM_SID, FILE_FULL_CONTROL),
...
def harden_runtime_tree(root: str | Path, sandbox_sid: str) -> None:
    ...
    if not runtime_root.is_dir():
        raise WindowsSandboxAclError(
            "WINDOWS_RUNTIME_ROOT_INVALID", "Runtime ACL root must be a directory"
        )
    grants = runtime_tree_grants(sandbox_sid)
    paths = [
        runtime_root,
        *sorted(runtime_root.rglob("*"), key=lambda value: len(value.parts)),
    ]
    for path in paths:
...
def verify_runtime_tree_dacl(root: str | Path, sandbox_sid: str) -> None:
    ...
    for path in [runtime_root, *runtime_root.rglob("*")]:
        if read_dacl(path) != (expected_directory if path.is_dir() else expected_file):
            raise WindowsSandboxAclError(
                "WINDOWS_RUNTIME_DACL_MISMATCH", "Runtime package DACL is not exact"
            )
...
def create_task_workspace(root: str | Path, task_id: str) -> tuple[Path, str]:
    ...
    if not workspace_root.is_absolute():
        raise WindowsSandboxAclError(
            "WINDOWS_ACL_PATH_RELATIVE", "Task workspace root must be absolute"
        )
