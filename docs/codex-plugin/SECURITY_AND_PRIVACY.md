...
- NormalizedEndpointV1 在摘要前执行：scheme/host 小写、IDN 转 punycode、缺省端口补为有效数字（https=443、http=80）、path 点段与百分号编码规范化、禁止 query/userinfo/fragment；https 是 model/TTS 默认，http 仅允许 Service 启动时固定的字面 IPv4 loopback AnkiConnect endpoint。端口可以由受控 launcher 配置以避开 Windows 排除端口范围，但不能由 MCP 调用方、项目或任务覆盖。ProfileConfigurationManifestV1 的字段按 capability 判别联合封闭，configurationFingerprint = SHA-256(JCS(manifest))，秘密值不进入 preimage，credentialRevision 独立绑定。
...
Windows launcher 的外层代码签名现有独立 fail-closed 验证器。它按微软 [`WINTRUST_DATA`](https://learn.microsoft.com/en-us/windows/win32/api/wintrust/ns-wintrust-wintrust_data) 的缓存专用 URL 和 chain-exclude-root 撤销标志调用 `WinVerifyTrust`，再通过 [`WTHelperProvDataFromStateData`](https://learn.microsoft.com/en-us/windows/win32/api/wintrust/nf-wintrust-wthelperprovdatafromstatedata)、[`WTHelperGetProvSignerFromChain`](https://learn.microsoft.com/en-us/windows/win32/api/wintrust/nf-wintrust-wthelpergetprovsignerfromchain) 和 [`WTHelperGetProvCertFromChain`](https://learn.microsoft.com/en-us/windows/win32/api/wintrust/nf-wintrust-wthelpergetprovcertfromchain) 提取 Windows 实际验证的 signer，而不是信任文件元数据或 payload 自报。外部 canonical policy 同时固定 signer DER digest/subject/status、代码签名 EKU、KeyUsage、密钥强度和时间戳要求；证书链、缓存撤销状态、pin 或文件稳定性任一无法证明都拒绝。当前 launcher 为 `NotSigned`，所以这条门槛的真实结论是阻断，不是“功能存在即通过”。正式证书、可信时间戳和生产 policy 仍必须来自仓库外发布流程。

原生 launcher 现进一步区分“被动候选”和“可安装构建”。被动构建没有安装策略 pin，并在任何 `.mcp.json`、安装 manifest/signature/policy 或 plugin MCP/App 声明出现时，于 Python 启动前退出 125；真实二进制注入与直接 `--verify-install-only` 探针已证明这一点。可安装构建必须显式编入候选目录之外的策略 SHA-256，并使用 `study.plugin-install-manifest.v1` 域验证 canonical install manifest 的 Ed25519 detached signature、有效期、key epoch/status、最低稳定版本与撤销集合；被动发布签名不能跨域复用。签名资源精确绑定固定 launcher 命令、MCP 配置、插件 manifest、SBOM、外层文件集合，以及委托运行包的 manifest/trust pins，内层资源随后再次全量复核。安装 finalizer 已实现无私钥/无联网的候选构建、public-key-only 请求、外部签名验证、WinTrust/AuthentiCode pin、隔离 staging、精确 DACL、原生 install-only 预检、原子发布和发布后复核；正式 API 不允许调用者注入伪验证器或回拨验证时间。正常 `--stdio` 运行在全部资源通过后才原子推进本机 canonical 回滚地板；install-only 预检不启动 Python也不推进地板。测试覆盖篡改、额外文件、过期、撤销密钥、版本降级、跨域签名、未签名 launcher、原生拒绝和地板负例。当前没有正式发布 key/HSM signature 或真实 Authenticode 正例，因此真实 finalizer 必须失败关闭，仍不得生成正式可安装包。
...
27. launcher、payload 和 hash 表一起被替换时外层签名失败；撤销密钥与降级包被拒绝。被动包手工加入 `.mcp.json` 必须在 Python 启动前退出 125；安装签名跨用被动域、过期、撤销 key、低于最低版本或出现未列文件时均失败。
