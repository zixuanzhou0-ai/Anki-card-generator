...
原生 launcher 现进一步区分“被动候选”和“可安装构建”。被动构建没有安装策略 pin，并在任何 `.mcp.json`、安装 manifest/signature/policy 或 plugin MCP/App 声明出现时，于 Python 启动前退出 125；真实二进制注入探针已证明这一点。可安装构建必须显式编入候选目录之外的策略 SHA-256，并使用 `study.plugin-install-manifest.v1` 域验证 canonical install manifest 的 Ed25519 detached signature、有效期、key epoch/status、最低稳定版本与撤销集合；被动发布签名不能跨域复用。签名资源精确绑定固定 launcher 命令、MCP 配置、插件 manifest、SBOM、外层文件集合，以及委托运行包的 manifest/trust pins，内层资源随后再次全量复核。该 verifier 已覆盖篡改、额外文件、过期、撤销密钥、版本降级和跨域签名负例；但当前没有正式发布 key、真实 Authenticode 正例、install finalizer 或原生单调 anti-rollback floor，因此仍不得生成正式可安装包。
