...
3. 10 个生产生成变体都用真实 `handle_export` 产物执行整包合同验证，覆盖当前 V15、历史 V14/V12 与明确支持的 V10 语言/文档路径，而不是只测试手工构造 fixture。
...
6. 真实生成的 V14 与 V10 APKG 进入 release smoke；V15 由生产 20 卡包、模型作用域 GUID 和完整包合同覆盖；V13、V199、近似名称、非规范 ID/整数和字段/模板/model extras/CSS 篡改进入负向合同测试。release smoke 强制调用生产 `verify_apkg.py`；验证器缺失、合同不唯一或报告失败都会阻断发布，独立 CLI 合同失败时 `ok=false` 且退出码为 1。
...
10. 最终自动化回归通过 Vitest 830、正式 `pytest` 603、独立 `unittest discover` 576、Rust 31 项通过与 1 项按设计忽略、UI smoke 3、V15/V10 release smoke、`npm run check:full` 和 `npm run tauri:build`。`pytest` 与 `unittest` 有重叠，不能相加为独立测试总数。
11. 20 卡生产 V15 离线 APKG 通过完整合同：20 notes / 20 cards / 52 个唯一媒体，manifest、逐媒体哈希、20/20 字幕对齐和模型作用域 GUID 闭合。素材是合成视频与静音 TTS，不是语义、听感或长期学习效果证据。
...
13. 最终 V15 20 卡包完成隔离 Anki 数据级核验：首次导入 20 notes / 20 cards / 52 media，逐媒体哈希闭合，重复导入跳过，真实重启后仍为 20/20/52；V14/V15 同字段 note 也在同一集合中保持不同 model、GUID 与 note/card ID。
14. Computer Use 在真实 Anki 26.05 中完成 20 张连续复习、正反面布局、滚动、焦点、四类媒体播放与互斥、Space/Enter 路由、重复导入和重启验收；媒体快捷键 add-on 只在版本化 runtime contract 精确匹配时工作，失配 fail closed。正式 profile/牌组未触碰。
...
这里的“已实现”覆盖当前 Worker/APKG、标准 Windows Anki direct-first 媒体路径、隔离 Anki 数据级基线，以及 Anki 26.05 的 Computer Use 真实 GUI/20 张连续复习；M0 已完成。`genanki` 固定不等于全部依赖已做版本+哈希供应链锁定。Card Service、MCP、Codex 插件安装、raw `ExportResult` 的 M2 认证替代、完整 Win32 目录句柄/私有 staging 与非标准 profile 的全流式兼容均不属于 M0 完成项。详见 [M0 验证报告](M0_VERIFICATION_REPORT_2026-07-17.md)。
