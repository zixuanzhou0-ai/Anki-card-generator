...
| PR-305 | Anki 明确授权 | CURRENT authenticated ImportPlan；PROPOSED internal approval ledger | prepare/confirm/import_and_verify | prepare 跨 audience/离线/幂等；后续 no-confirm/session-replay negative | M3 |
...
M0 已完成；M1 Card Service、最小 stdio MCP、固定原生 launcher 与若干 M2 内核/资源入口切片已实现。CURRENT 已开放严格受限的确定性文本 `cards.generate`/`cards.list`、异步 `cards.export_apkg`、任务查询/取消与只读 `anki.prepare_import`，产出认证 ProjectArtifact/PackageArtifact/ImportPlan；但 Skill/App UI、生产签名安装包、模型/TTS/媒体生成、Anki 受信确认/写入工具和 Codex→Anki 端到端仍未完成。非标准 inline 兼容路径与双进程 RSS、Anki add-on 仅声明支持 26.05、合成媒体不证明真人学习效果仍是明确限制。RR-000/RR-601/RR-801 的证据不能外推为 RR-802 通用 runtime 或插件已经交付。
