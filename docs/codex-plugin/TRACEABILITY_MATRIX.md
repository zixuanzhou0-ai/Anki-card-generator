...
当前 M2 内部证据：PR-002 的 ArtifactEnvelope/认证 Registry 与 legacy Project 非秘密 canonical projection、PR-205/402/403/404 的 StudyTask/检查点/successor、PR-207 的 Project/Contract 双 revision、语义操作、幂等与固定失效矩阵、SR-013 的 model/TTS OperationIntent/Approval/Authorization 账本内核、PR-406/502/504 的认证 SecretRef/credentialRevision、持久非秘密 Service Profile Registry 与逐 profile verification registry，以及 PR-001/SR-002/SR-003/SR-018 的 local/network resource registry 均已有服务端实现和自动化测试。本地资源链还覆盖认证 resolution proof、task-bound staging receipt、文件/目录有界复制、逐项 manifest、竞态/篡改复核和 Windows task SID 只读 DACL；Card Service 已惰性组合单一 local grant/staging runtime，以 OS-backed credential 根材料派生不落盘的域隔离 key，service instance ID 每次启动随机生成，并在 packaged runtime 强制正式 hardener、禁止任意 verifier 注入。真实本地 picker 以 session/nonce/surface 绑定的 AES-GCM 响应、5 分钟精确 attestation 和状态根拒绝签发 opaque ref；公开结果不含 path/ciphertext/attestation。网络账本覆盖 raw URL 仅进程内、脱敏持久摘要、全部地址公网判定、DNS rebinding、同源重定向、固定 IP + TLS hostname、无 ambient auth、限额、幂等、过期、撤销和 HMAC 防篡改。staging 切片的 M2 十一组件联合回归为 266 passed；受信 picker/资源/打包聚类为 144 passed，正式 Python 全集为 1247 passed、1 skipped。宿主附件与 URL 输入生产 attestation、StudyTask/Worker locator 绑定、受信设置/验证任务、逐子项公共 ref、网络 staging、ImportApproval、公共 MCP 工具、统一跨 Registry 事务和正式安装边界仍未完成，表中 M3 出口不因此提前满足。
