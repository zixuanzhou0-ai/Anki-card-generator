...
M0 已完成；M1 Card Service、最小 stdio MCP、固定原生 launcher 与若干 M2 内核/资源入口切片已实现。CURRENT 已开放严格受限的确定性文本 `cards.generate`/`cards.list` 并产出认证 ProjectArtifact，但 Skill/App UI、生产签名安装包、模型/TTS/媒体生成、公共 APKG 导出、Anki 写工具和 Codex→Anki 端到端仍未完成。非标准 inline 兼容路径与双进程 RSS、Anki add-on 仅声明支持 26.05、合成媒体不证明真人学习效果仍是明确限制。RR-000/RR-601/RR-801 的证据不能外推为 RR-802 通用 runtime 或插件已经交付。
