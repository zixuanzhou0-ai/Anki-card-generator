...
4. Service 将 YouTube 地址缩减为规范化 videoId；跟踪 query 不进入后续请求、Artifact 或日志。yt-dlp 远程组件保持禁用。
5. CURRENT 只获取现有字幕并发布 VTT 快照，再走字幕检查与候选发现；不下载视频、原声或自动转写。
6. 无稳定字幕时明确阻塞，不能把页面标题或模型猜测伪装成视频内容。
7. 只有字幕证据覆盖且卡片路线不要求未实现媒体时才能继续生成文本卡。
8. 同一运行期的精确幂等重试可复用；Service 重启后必须重新在受信表面授权 URL。

### 失败分支

- 跳转私网：阻塞并解释。
- 视频不可访问或没有可用字幕：不发布虚假的可学习来源，也不显示找到候选。
