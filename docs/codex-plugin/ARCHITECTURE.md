...
- M0 最终证据保存在独立验证报告；最新 M1 分支证据为正式 `pytest tests` 943 项、Vitest 830 项、Worker `unittest discover` 598 项、原生插件 launcher 12 项 Rust 单元测试与严格 clippy、托管 yt-dlp launcher 2 项 Rust 测试，以及 Tauri 31 项通过、1 项按设计忽略。各集合有重叠，不相加；完整发布回归仍以路线图出口为准。
...
- M1 原生安装 verifier 使用 `ed25519-dalek 3.0.0` 对 `study.plugin-install-manifest.v1` 域执行 strict verify；策略、manifest、signature 与 MCP 元数据均要求有界 canonical JSON，路径必须可移植且无 reparse/碰撞/保留名，外层文件集合必须精确。两个隔离 `/Brepro` 构建得到相同 314,368-byte launcher（SHA-256 `2dffc71f4b72a0224935e9f5f031b95fb7baa29073564827578c317cc781e65a`）；新版 4391 项被动候选为 289,216,289 bytes，manifest `a6f597824355f2648e272241bf2e8f6514ae138807ca96e8e4e4a3a809b28ea7`，官方 validator、真实 Codex 宿主和伪造 `.mcp.json` 退出 125 的负向实物均通过。该候选仍没有正式外层签名，不能安装。
