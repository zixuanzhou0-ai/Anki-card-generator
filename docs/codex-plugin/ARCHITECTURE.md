...
- CURRENT 内部 AuthorizationLedger 把 OperationIntent、一次性 OperationApproval 消费、task-bound InternalAuthorization、逐调用 usage 和共享 operation remote-call 上限保存在一个 HMAC 认证、文件锁串行化的原子记录中。Task manifest binding 与内部 authorizationId 分离；未配置受信手势 attestation verifier 时所有批准/撤销写入失败关闭。本地资源 picker 的正式 verifier 已接入独立资源账本，并已通过可信 stdio 暴露 source/output grant；固定 Hermes candidate discovery 授权和异步执行已经接线。Anki ImportApproval 使用专用 HMAC 账本、受信窗口和 audience/session 绑定的单次消费合同，`anki.import_and_verify` 已消费该意图执行幂等写入和数据核验。资源账本和 ImportApproval 具有有界、认证、按 audience 隔离的内部枚举；ImportApproval 撤销与消费在同一文件锁事务下只能有一个赢家，活动 Broker 的全部 model/TTS/source profile 也能在一个 ledger 写入中撤销并立即从 Service 解绑。`system.revoke_grant` 现已通过独立受信本地授权管理器公开：MCP 只能打开/轮询窗口，私有 locator 只在 Service 内存绑定，窗口返回的选择经 AES-GCM 与 request/session/surface 绑定，撤销 attestation 逐目标、短期且 audience-bound；Broker 额外绑定打开窗口时的 authorization digest，阻止旧窗口撤销新授权。当前仍不宣称跨资源/Anki/Broker Registry 的全局事务，也不覆盖尚未公开的通用 OperationApproval、URL 输入或 runtime verifier。
