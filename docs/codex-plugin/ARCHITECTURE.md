...
- M0 最终证据保存在独立验证报告；最新 M1 分支证据为正式 `pytest tests` 949 项、Vitest 830 项、Worker `unittest discover` 598 项、原生插件 launcher 13 项 Rust 单元测试与严格 clippy、托管 yt-dlp launcher 2 项 Rust 测试，以及 Tauri 31 项通过、1 项按设计忽略。各集合有重叠，不相加；安装最终化实物证据见 [M1 安装最终化验证报告](M1_INSTALL_FINALIZER_VERIFICATION_2026-07-18.md)，完整发布回归仍以路线图出口为准。
