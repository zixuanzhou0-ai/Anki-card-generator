...
- 该 composition root 仍不是完整事务边界：新公开 ref 尚未由 `study.register_inputs` 绑定到 StudyTask、Source Adapter 或 Worker 调度，output ref 也未绑定 APKG 原子发布；宿主附件、受信 URL 输入、网络 staging、逐子项公共 ref、生成、导出与 Anki 写工具仍未接线。Artifact publish 失败可能留下已经净化的内容寻址孤儿 Blob；跨 Registry 原子事务和保留清理属于后续 M2。
