...
| 学习硬门禁可机器审计 | 已加入 GateEvaluationSet、规则版本、revision 与 stale 语义 | CURRENT `candidate-gates-language-v1` 内核、认证 CandidateProposal → GateEvaluation → Discovery 图，以及角色/schema 分离的内部提案-复核引擎；公共 discovery、真实 Broker/Approval/任务接线与 benchmark 待实现 |
