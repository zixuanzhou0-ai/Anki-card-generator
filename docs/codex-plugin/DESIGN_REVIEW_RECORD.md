...
| 学习硬门禁可机器审计 | 已加入 GateEvaluationSet、规则版本、revision 与 stale 语义 | CURRENT `candidate-gates-language-v1` 内核与认证 CandidateProposal → GateEvaluation → Discovery 图；公共 discovery、独立模型复核接线与 benchmark 待实现 |
