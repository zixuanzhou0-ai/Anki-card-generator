...
`system.revoke_grant` 只在用户明确要求管理权限时打开独立受信本地窗口。窗口从当前 audience 的认证账本读取脱敏资源、未消费 Anki 批准和当前 Broker 授权，真实用户在窗口中选择后才生成短期、精确绑定的撤销证明；MCP 不接受资源、导入、profile、ledger 或内部 authorization ID。撤销阻止后续使用，但不会伪装成回滚已经完成的读取、远程调用、Artifact 或 Anki 写入。
