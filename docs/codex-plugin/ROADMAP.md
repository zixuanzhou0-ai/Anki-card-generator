...
- 本里程碑仍未完成：M2 的逐 OperationIntent approval ledger、撤销/原子消费和正式 profile 注册尚未实现；公网 provider 也未做真实用户凭据调用，Vertex TTS OAuth egress 仍 fail closed。完整 YouTube 视频/音频获取仍未开放，托管模式不会静默退回直接 yt-dlp；实物解码炸弹/恶意循环容器 corpus、安全 Artifact 保留清理、更多桌面/Headless 语义等价证据和正式外层插件 launcher/可安装包仍在后续切片。
- 当前回归证据包括正式 Python 全集 864 项、正式 Worker `unittest discover` 594 项、Vitest 830 项，以及 lint、类型检查和生产构建，以及真实受限 Legacy Worker 的模型、TTS、字幕→模型三条认证 stdio 闭环和一条桌面/Headless 单卡语义等价合同；这些集合有重叠且不是 M1 完成判定。
