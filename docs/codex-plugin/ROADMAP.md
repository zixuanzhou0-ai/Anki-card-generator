...
- 已实现原生 pinned launcher 与离线构建器：launcher 把精确 runtime manifest/trust-policy 摘要编入自身，启动前拒绝 reparse/路径逃逸/额外文件并流式复核全部 4380 项 size/SHA-256，只接受 `--stdio`，只启动相对位置的包内 Python。构建使用 Cargo.lock、`--offline`、`/Brepro`、固定 epoch 和禁用增量；平台识别修复后的两个全新 target 得到相同的 308,736-byte 二进制，SHA-256 为 `c11b912b0590c406aef361400820e776ae7d009b85c8b7019f2e912d9caed3c3`。launcher 为受限宿主固定 AMD64 构建架构，未来运行包同时从 Python `sysconfig` 标签判定平台。独立复制插件目录通过官方 validator，真实 Codex `0.144.1` 从该 launcher 完成唯一只读工具调用；替换 trust policy 或运行资源均在启动 Python 前退出 125。正式 Authenticode/等价外层签名和发行密钥管理仍未完成，因此仓库插件继续不声明 `.mcp.json`。
- 已实现外层被动离线候选包构建器与验证器：只接受不声明 MCP/App 的插件、固定 launcher、已签名 runtime 和独立 trust policy，生成覆盖精确 4391 项的 canonical 外层 manifest 与 SPDX，并对外层/运行时分别应用和复核精确 Windows DACL。实物为 289,210,657 bytes，manifest SHA-256 `9ab03b75dc2a0c2197e280fdd47a9f9a83c2fe25fc786459822cfcbf23f262d6`；普通权限官方 validator 与真实 Codex 候选目录宿主调用均通过。候选固定 `installable=false`、`mcpDeclared=false`、`outerSignatureVerified=false`、`publisherKeyManaged=false`，不是可安装发布物。
...
- 当前回归证据包括正式 Python `tests/` 全集 942 项、正式 Worker `unittest discover` 598 项、Vitest 830 项，以及 lint、类型检查和生产构建，以及真实受限 Legacy Worker 的模型、TTS、字幕→模型三条认证 stdio 闭环和一条桌面/Headless 单卡语义等价合同；这些集合有重叠且不是 M1 完成判定。原生插件 launcher 另有 5 项 Rust 单元测试、严格 clippy 和 release 构建，托管 yt-dlp launcher 为 2 项 Rust 测试，Tauri 为 31 项通过、1 项按设计忽略。裸 `pytest` 会误收集历史 `target/test-results/tmp` 运行包副本，因此正式范围固定为 `pytest tests`，不通过删除历史证据来获得假绿。
