...
- 已实现原生 pinned launcher 与离线构建器：launcher 把精确 runtime manifest/trust-policy 摘要编入自身，启动前拒绝 reparse/路径逃逸/额外文件并流式复核全部 4380 项 size/SHA-256，只接受 `--stdio`，只启动相对位置的包内 Python。构建使用 Cargo.lock、`--offline`、`/Brepro`、固定 epoch 和禁用增量；最终两个全新 target 得到相同的 308,736-byte 二进制，SHA-256 为 `3ae189f968a34c3c850dbab4aa00cec9b834f6d89deb3f759bd130bde1adb1fa`。独立复制插件目录通过官方 validator，真实 Codex `0.144.1` 从该 launcher 完成唯一只读工具调用；替换 trust policy 或运行资源均在启动 Python 前退出 125。正式 Authenticode/等价外层签名和发行密钥管理仍未完成，因此仓库插件继续不声明 `.mcp.json`。
...
- 本里程碑仍未完成：M2 的逐 OperationIntent approval ledger、撤销/原子消费和正式 profile 注册尚未实现；公网 provider 也未做真实用户凭据调用，Vertex TTS OAuth egress 仍 fail closed。完整 YouTube 视频/音频获取仍未开放，托管模式不会静默退回直接 yt-dlp；更广泛的 codec parser 崩溃/fuzz corpus、安全 Artifact 保留清理、更多桌面/Headless 语义等价证据，以及正式发布密钥、外层 Authenticode/等价签名与离线安装包仍在后续切片。真实 launcher 宿主探针证明独立副本能够注册并完成只读调用，但不等于插件已安装，也不等于发行者身份和供应链出口完成。
- 当前回归证据包括正式 Python `tests/` 全集 914 项、正式 Worker `unittest discover` 598 项、Vitest 830 项，以及 lint、类型检查和生产构建，以及真实受限 Legacy Worker 的模型、TTS、字幕→模型三条认证 stdio 闭环和一条桌面/Headless 单卡语义等价合同；这些集合有重叠且不是 M1 完成判定。原生插件 launcher 另有 5 项 Rust 单元测试、严格 clippy 和 release 构建，托管 yt-dlp launcher 为 2 项 Rust 测试，Tauri 为 31 项通过、1 项按设计忽略。裸 `pytest` 会误收集历史 `target/test-results/tmp` 运行包副本，因此正式范围固定为 `pytest tests`，不通过删除历史证据来获得假绿。
