...
- 托管 FFmpeg 现增加执行前资源证据闸门：同一命令输入合计最多 8 GiB、32 个流、12 小时，视频限制单轴 8192、每帧最多 8192×4320 像素、240 fps、300 万帧及 16 TiB 逻辑解码量，音频限制 192 kHz、8 声道及 64 GiB 逻辑解码量；FFprobe 输出、探测耗时和码率也有限。输入在探测期间变化会拒绝，所有输出统一施加 512 MiB `-fs` 硬上限，达到上限的半成品会删除。真实正常 WAV/MP4 与伪造的时长、分辨率、帧率、采样率、流数、附件和输出填充负例均通过；完整实物 decode-bomb corpus 与任务级磁盘配额仍未完成。
...
- 本里程碑仍未完成：M2 的逐 OperationIntent approval ledger、撤销/原子消费和正式 profile 注册尚未实现；公网 provider 也未做真实用户凭据调用，Vertex TTS OAuth egress 仍 fail closed。完整 YouTube 视频/音频获取仍未开放，托管模式不会静默退回直接 yt-dlp；扩展恶意媒体/资源耗尽 corpus、更多桌面/Headless 语义等价证据和正式外层插件 launcher/可安装包仍在后续切片。
- 当前回归证据包括正式 Python 全集 844 项、正式 Worker `unittest discover` 587 项，以及真实受限 Legacy Worker 的模型、TTS、字幕→模型三条认证 stdio 闭环和一条桌面/Headless 单卡语义等价合同；这些集合有重叠且不是 M1 完成判定。
