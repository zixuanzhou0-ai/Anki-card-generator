...
- 当前回归证据包括正式 Python 全集 841 项、正式 Worker `unittest discover` 584 项，以及真实受限 Legacy Worker 的模型、TTS、字幕→模型三条认证 stdio 闭环和一条桌面/Headless 单卡语义等价合同；这些集合有重叠且不是 M1 完成判定。
