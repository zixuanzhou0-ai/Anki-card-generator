...
`anki.prepare_import` 随后进入 CURRENT：它只接受当前 PackageArtifact opaque handle，流式复核 APKG Blob，固定只读探测本机 loopback AnkiConnect，并发布认证 11 项数据验证合同与 ImportPlan。精确重试保持同一计划句柄，跨插件/旧包/离线目标失败关闭，公开结果不含 profile 名、媒体目录或 deck 名。该切片尚未创建 ImportApproval，也不写入 Anki；下一出口是受信本地确认与一次性 `importIntentId`。新增 16 项快速契约和 1 条真实 APKG→ImportPlan 重型链已通过。

Artifact Registry、StudyTask、Project Registry、AuthorizationLedger、CredentialStore、ServiceProfileRegistry、profile verification、legacy projection 与 local/network resource ledger 仍未作为同一事务边界接入 Card Service 公共 MCP surface，Learning Contract 尚未发布为 canonical Artifact ref。正式 service key rotation、完整应用数据 ACL、Codex 宿主进程身份证明、精细 scope relation proof、完整 Broker reservation 与授权联合事务、受信设置事务、`system.validate_profile` 公共任务和 Anki 证据链仍在后续切片。公共 MCP 在这些边界完成前仍不能接受任意 ArtifactRef 对象、扩大到模型/TTS/媒体生成或开放 Anki 写操作；已经开放的确定性 `cards.generate` 与 `cards.export_apkg` 只能通过 audience 绑定 opaque handle、当前项目 revision 和受信资源授权。
