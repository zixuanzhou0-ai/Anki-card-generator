...
- 已实现原生 pinned launcher 与离线构建器：launcher 把精确 runtime manifest/trust-policy 摘要编入自身，启动前拒绝 reparse/路径逃逸/额外文件并流式复核全部 4380 项 size/SHA-256，只接受 `--stdio`，只启动相对位置的包内 Python。构建使用 Cargo.lock、`--offline`、`/Brepro`、固定 epoch 和禁用增量；加入安装清单闸门后的两个全新 target 得到相同的 314,368-byte 二进制，SHA-256 为 `2dffc71f4b72a0224935e9f5f031b95fb7baa29073564827578c317cc781e65a`。launcher 为受限宿主固定 AMD64 构建架构，未来运行包同时从 Python `sysconfig` 标签判定平台。独立复制插件目录通过官方 validator，真实 Codex `0.144.1` 从该 launcher 完成唯一只读工具调用；替换 trust policy、运行资源或向被动包注入 `.mcp.json` 均在启动 Python 前退出 125。正式 Authenticode/等价外层签名和发行密钥管理仍未完成，因此仓库插件继续不声明 `.mcp.json`。
- 已实现外层被动离线候选包构建器与验证器：只接受不声明 MCP/App 的插件、固定 launcher、已签名 runtime 和独立 trust policy，生成覆盖精确 4391 项的 canonical 外层 manifest 与 SPDX，并对外层/运行时分别应用和复核精确 Windows DACL。使用新版被动闸门的实物为 289,216,289 bytes，manifest SHA-256 `a6f597824355f2648e272241bf2e8f6514ae138807ca96e8e4e4a3a809b28ea7`；普通权限官方 validator 与真实 Codex 候选目录宿主调用均通过。候选固定 `installable=false`、`mcpDeclared=false`、`outerSignatureVerified=false`、`publisherKeyManaged=false`，不是可安装发布物；旧候选外层签名不能复用于该新 manifest。
