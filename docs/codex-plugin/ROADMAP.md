...
- 已实现原生 pinned launcher 与离线构建器：launcher 把精确 runtime manifest/trust-policy 摘要编入自身，启动前拒绝 reparse/路径逃逸/额外文件并流式复核全部 4380 项 size/SHA-256，只接受 `--stdio`，只启动相对位置的包内 Python。构建使用 Cargo.lock、`--offline`、`/Brepro`、固定 epoch 和禁用增量；加入安装清单与防回滚闸门后的两个全新 target 得到相同的 314,880-byte 二进制，SHA-256 为 `81dbd78fe58fd5d1bd68488cfb1a83b4e88eb13f7071186eb7048c41b1e86ac1`。launcher 为受限宿主固定 AMD64 构建架构，未来运行包同时从 Python `sysconfig` 标签判定平台。独立复制插件目录通过官方 validator，真实 Codex `0.144.1` 从该 launcher 完成唯一只读工具调用；替换 trust policy、运行资源或向被动包注入 `.mcp.json` 均在启动 Python 前退出 125。正式 Authenticode/等价外层签名和发行密钥管理仍未完成，因此仓库插件继续不声明 `.mcp.json`。
- 已实现外层被动离线候选包构建器与验证器：只接受不声明 MCP/App 的插件、固定 launcher、已签名 runtime 和独立 trust policy，生成覆盖精确 4391 项的 canonical 外层 manifest 与 SPDX，并对外层/运行时分别应用和复核精确 Windows DACL。使用防回滚版被动闸门的实物为 289,216,801 bytes，manifest SHA-256 `0eef63f597517ce5c40c525169631a8ec3fb3f9689d335f7fe95e945539aa375`；普通权限官方 validator 与真实 Codex 候选目录宿主调用均通过。候选固定 `installable=false`、`mcpDeclared=false`、`outerSignatureVerified=false`、`publisherKeyManaged=false`，不是可安装发布物；旧候选外层签名不能复用于该新 manifest。
...
- 已把“可安装接线”移入原生启动器的独立签名域：只有构建时显式 pin 外部发布策略的 launcher 才接受 `install-package-v1.json`/detached signature，并精确校验 `.mcp.json` 固定命令、plugin manifest、SBOM、外层资源及委托 runtime pins；被动 launcher 会拒绝任何安装元数据或 MCP/App 声明。外层与 runtime 全部验证后才原子推进本机防回滚地板。13 项 Rust 测试覆盖 canonical/路径/资源、篡改、额外文件、过期、撤销 key、版本降级、跨域签名、被动绕过和回滚地板。正式 install finalizer、生产发布策略/签名和独立安装验收仍未完成，因此此切片不改变可安装状态。
...
- 已新增离线、确定性、原子且拒绝覆盖已有目录的正式 staging 构建器 `scripts/build_managed_runtime.py`：只收集显式仓库源码、预装配 Python 运行时、完整 Python lock 和固定 FFmpeg/ffprobe/yt-dlp，逐文件复制后复核 SHA-256，生成精确 SPDX 2.3 与 canonical manifest。构建前必须读取有界、canonical 的 `python-runtime-build-v1.json`，证明该便携 Python 的 lock SHA-256 与 wheel 数精确匹配所给 25 项锁；构建元数据本身以 `managed-python:build-metadata` 纳入签名 manifest，运行包验签时再次与 `metadata:python-runtime-lock` 交叉验证，不能再把两个分别有效但彼此无关的产物拼接。包内 Python 模块布局固定为顶层 `card_service/` 与 `workers/`，不依赖 `PYTHONPATH` 注入。相同输入与显式 UTC 时间产生相同 manifest/SBOM；构建器不接收、读取或输出发布私钥，也不在运行时联网下载组件。manifest、SBOM、Python lock、Python build metadata、签名、信任策略和回退 floor 均先检查文件大小再有界读取，避免超大元数据在拒绝前耗尽内存。
...
- 本里程碑仍未完成：M2 的逐 OperationIntent approval ledger、撤销/原子消费和正式 profile 注册尚未实现；公网 provider 也未做真实用户凭据调用，Vertex TTS OAuth egress 仍 fail closed。完整 YouTube 视频/音频获取仍未开放，托管模式不会静默退回直接 yt-dlp；更广泛的 codec parser 崩溃/fuzz corpus、安全 Artifact 保留清理、更多桌面/Headless 语义等价证据，以及正式发布密钥、外层 Authenticode/等价签名与正式可安装包仍在后续切片。真实候选包宿主探针证明完整被动目录能够注册并完成只读调用，但不等于插件已安装，也不等于发行者身份和供应链出口完成。
- 当前回归证据包括正式 Python `tests/` 全集 943 项、正式 Worker `unittest discover` 598 项、Vitest 830 项，以及 lint、类型检查和生产构建，以及真实受限 Legacy Worker 的模型、TTS、字幕→模型三条认证 stdio 闭环和一条桌面/Headless 单卡语义等价合同；这些集合有重叠且不是 M1 完成判定。原生插件 launcher 另有 13 项 Rust 单元测试、严格 clippy 和 release 构建，托管 yt-dlp launcher 为 2 项 Rust 测试，Tauri 为 31 项通过、1 项按设计忽略。裸 `pytest` 会误收集历史 `target/test-results/tmp` 运行包副本，因此正式范围固定为 `pytest tests`，不通过删除历史证据来获得假绿。
