...
- 当前回归证据包括正式 Python `tests/` 全集 930 项、正式 Worker `unittest discover` 598 项、Vitest 830 项，以及 lint、类型检查和生产构建，以及真实受限 Legacy Worker 的模型、TTS、字幕→模型三条认证 stdio 闭环和一条桌面/Headless 单卡语义等价合同；这些集合有重叠且不是 M1 完成判定。原生插件 launcher 另有 5 项 Rust 单元测试、严格 clippy 和 release 构建，托管 yt-dlp launcher 为 2 项 Rust 测试，Tauri 为 31 项通过、1 项按设计忽略。裸 `pytest` 会误收集历史 `target/test-results/tmp` 运行包副本，因此正式范围固定为 `pytest tests`，不通过删除历史证据来获得假绿。
