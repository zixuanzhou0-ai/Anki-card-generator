...
CURRENT 内部发现实现把模型参与拆成 `high-recall-language-proposer-v1` 与 `independent-learning-reviewer-v1` 两个封闭角色。前者只看通过披露筛选的认证来源窗口，只能返回目标形式、候选类型、语义功能、路线和精确 spans；后者只看安全提案、目标 quote、同一披露窗口内最多 240 字符的前后文和 Learning Contract，只能返回语义支持、冲突、learner fit 与简短理由。两个角色都无权写 eligibility、scores、GateResult、重复标记、选择或锁定状态；确定性服务门禁才派生这些字段。独立在这里表示角色、上下文和 schema 独立，并不要求不同供应商。敏感来源节点在模型调用前剔除，越界 span 拒绝，安全失败提案不进入复核，缺少复核结果派生 `needs_review`。ProposalBatch 与 ReviewBatch 是 CandidateProposal/GateEvaluation 的认证父 Artifact；真实 Broker、可恢复发现任务、三个只读候选查询与本地认证选择写入已经接线，异步公共 discovery 仍须单独完成。

CURRENT `CandidateDiscoveryRuntime` 只接受受信服务边界已经解析出的非秘密授权证明，并把 Inspection ref、来源快照摘要、合同修订、候选预算、模型 profile/configurationFingerprint/credentialRevision/实现版本、egress manifest、OperationIntent、授权记录/constraints/exact scope/revocation epoch 和成本预算纳入同一个 TaskInputFingerprint。任务成功后才以 expected project revision 提交全部 ProposalBatch、ReviewBatch、Candidate、GateEvaluation 与 Discovery refs，将项目原子推进到 `candidates_ready`；若仅项目提交中断，重试复用 succeeded task。CURRENT 的 StudyTask 仍把 proposal/review/gate publication 视为一个原子 discovery work unit，失败后需要显式恢复；公共 MCP 不能接收或伪造上述内部授权字段。任务级 Service Broker 适配器在确定 taskId 后才绑定 handler，proposer 与 reviewer 分别使用 `candidate-proposer-v1` 和 `candidate-reviewer-v1`，从而让 reservation/idempotency 不相撞。模型身份在任务创建前冻结，绑定后若 profile/configurationFingerprint/credentialRevision/implementationVersion 任一变化则失败关闭。授权证明由 Card Service 对当前可信 Broker 清单、audience、project revision、inspectionHandle 与 candidateBudget 做 canonical digest 派生；Broker 在发送前仍独立复核过期、凭据修订、profile 撤销和剩余预算。

CURRENT 公共候选投影由 `CandidateQueryRuntime` 从当前项目最新 Discovery 的认证图重新组合，而不是信任调用方提供的 eligibility、gate、source 或 selection 状态。分页 cursor 使用独立派生密钥认证，并绑定 service instance、Discovery digest、规范化 filter/sort 与末项 candidateId；candidate 详情必须证明 candidateHandle 属于同一 Discovery。Evidence preview 只重放 SourceRepresentation 的内容寻址正文，复核 node bounds、quote SHA-256 与整 node 敏感披露策略，最多返回同一 node 内 480 字符前后文。公共结果只含 opaque handle、语义投影和无路径来源摘要，不含 ArtifactRef、BlobRef、RegistryAuthRef、InputFingerprint、模型授权或路径。CURRENT `CandidateSelectionRuntime` 只接受同一最新 Discovery 的 audience/session opaque handles，重新验证候选 membership、精确 Candidate ArtifactRef 和 eligibility/hard gates，再以 expected project revision 发布认证 `study.portfolio-selection`。显式 add/remove 保留用户决定；`accept_recommended` 只从 recommended 候选执行 `portfolio-coverage-v1` 确定性组合，综合迁移价值、路线与来源覆盖、饱和、语义重复和 review cost，并以 candidateId 稳定破同分。`needs_review` 仅能显式加入并产生警告；hard_blocked/excluded/duplicate 和 evidence/conflict/security fail 始终拒绝。选择任务不调用模型、TTS、网络或 Anki；查询层从当前且仍绑定同一 Discovery 的 SelectionArtifact 派生 selectionState，上游预算/输入失效使项目退回后不复用旧选择。
...
PackageArtifact 固定绑定原 ProjectArtifact、ReliabilityManifest、APKG SHA/size、文件名、deck、note/card/media 数量、front/back/CSS 摘要、媒体清单 digest、CardIdentitySet digest 和投递状态。公开结果只返回 PackageArtifact handle 与有界摘要，不返回 APKG path、Worker export path、内部 ArtifactRef、BlobRef、input fingerprint、媒体目录或 registry authentication data。当前实现阶段已包含 `apkg_ready`、认证 ImportPlan 与 session-bound ImportApproval；后续 AnkiVerificationArtifact 和实际写任务尚未开放。