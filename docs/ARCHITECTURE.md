...
- The review merge path is intentionally stricter than the prompt. `exact_span` must be recoverable from the source sentence, `answer_core` must be a clean English answer instead of a mixed explanation, and each `source_segment_id` keeps at most two learning points after ranking by value, score, and candidate kind. AI-rejected candidates are not revived by the local minimum-count fallback.
