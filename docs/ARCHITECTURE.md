...
```mermaid
flowchart LR
  A["Source: URL, local video + SRT, or document"] --> B["Subtitle/document parsing"]
  B --> C["Candidate segment building"]
  C --> D["Phrase mining and MIMO review"]
  D --> E["Card field generation"]
  E --> F["Media slicing and optional TTS"]
  F --> G["APKG packaging"]
  G --> H["APKG verification and Anki import"]
```
...
- Left Inspector: source input, learning range, card/template preferences, and readiness summaries.
