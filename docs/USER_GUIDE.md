...
- `候选片段` 是从字幕里挖出的学习机会总数，包含后续被拒绝和合并的内容。
- `推荐 / 待审 / 已拒绝 / 重复合并` 是 AI 评审和本地硬校验后的质量分类。
- `有效卡片` 是当前项目里可选择的卡片数量；顶部绿色导出条里的“已导出 N 张”是这次实际写进 APKG 的数量，可能因为你手动全选、勾选待审或保留了旧项目选择状态而大于“推荐保留”。

当前评审会做几道硬门槛：AI 返回的 `exact_span` 必须真的出现在原句里，`answer_core` 必须是纯英文学习答案，不能把中文释义、音标或解释塞进去；同一句字幕最多保留 2 个学习点。这样会牺牲一点数量，但能减少“词伙没完整显示”“表达词义塞了一堆说明”“一条字幕拆出过多低质卡”的情况。
...
导入核验会按导出结果里的模板版本查找 Anki 标签。V10 包使用 `anki_card_generator_v10`，V11 包使用 `anki_card_generator_v11`，所以升级模板后不会再用旧标签误判导入失败。
...
导出前先看卡片详情里的学习点和核心答案。表达小喇叭会使用“可见的核心答案”，整句朗读会使用英文原句。不要把中文解释、音标或“发音融合为...”这类说明写进核心答案；这些内容应该放在“别误用 / 老师提醒”里。最新评审会自动拒绝这种混合答案，但手动编辑时仍要保持这个边界。
