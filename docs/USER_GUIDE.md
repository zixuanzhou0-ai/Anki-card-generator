...
> v0.9.9-beta 起，安装版启动和 worker 调用外部工具时都应隐藏 Windows console。若仍看到黑色弹窗，请先确认不是旧版安装包、开发调试入口 `desktop:dev:debug`，或其它后台工具窗口。

## 2.1 统一学习卡策略

v0.9.9-beta 起，普通视频制卡默认只生成一种 `学习卡`。应用仍会在学习点阶段区分表达、语境生词、语法框架、听力难点和语气风险，但这些信息会合并到同一张卡的字段里，而不是为同一个学习点拆成听力卡、表达卡、填空卡等多张相似卡。

推荐逻辑也更严格：目标答案必须能在原句中清楚定位；`talk about`、`do something` 这类迁移价值低的泛表达会进入候选或待审，不会默认推荐导出。

如果模型没有完整返回某个已勾选学习点，应用会生成一张“保底卡”留在审核区，方便你知道这个学习点没有被完整处理。这类卡会标记为 `需修复`，默认不可导出；请重新生成、换更高质量模型，或手动修正字段后再导出。
