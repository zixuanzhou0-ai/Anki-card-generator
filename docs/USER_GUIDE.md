...
2. `学习设置`：选择学习语言、自动/手动难度、卡片类型、模板和预览偏好。
...
- `抽取学习点`：先让 AI 从字幕/文档里精筛词伙、口语、单词用法、语法和听力点。
- `导出`：生成完成后导出已选卡片。
...
| 模板 | 说明 |
| --- | --- |
| 沉浸复读 V11 | 默认稳定模板，偏沉浸跟读、释义、误用提醒和再造句 |
| 词霸天下实验 V1 | 实验风格，底层更重视词块、语境义、概念视角、搭配边界、真实听辨和“为说而思考” |

注意：`词霸天下实验 V1` 现在会导出独立 Anki 卡面。正面优先用召回问题，背面按 `核心答案 / 语境义 / 语言动作 / 迁移句 / 搭配边界 / 原句场景` 组织。它仍复用现有字段 schema，仍属于实验模板。
...
只有用户勾选的学习点才会进入完整制卡。这样能避免一开始就为所有候选生成 TTS、媒体和完整 Anki 字段。
