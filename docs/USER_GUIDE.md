...
如果只是视频下载失败，但字幕能下载，改用“只用字幕生成”。这种导出的卡包不包含视频片段和原声音频，但可以继续保留英文原句、词伙解释和 TTS。
...
建议使用“自动片段”。软件会先生成更多候选，再由 MIMO 评审过滤，不靠重复内容凑数量。生成后看右侧质量仪表盘：它会显示候选数、推荐数、待审数、拒绝数、重复合并数和平均词伙评分。

### 左侧设置太多

v0.9.2-beta 起，左侧 Inspector 会把不常用的选项收进展开项。常用流程只需要关注“素材”“学习设置”和右上角“生成卡片”；需要调下载策略、难度范围或模板时再展开对应条目。
