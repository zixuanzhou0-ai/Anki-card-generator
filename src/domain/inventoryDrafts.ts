...
export function draftCardFromInventoryItem(item: LearningPointInventoryItem): Card {
  ...
  return {
    ...
    exact_span_start: item.exact_span_start ?? undefined,
    exact_span_end: item.exact_span_end ?? undefined,
    normalized_answer: item.normalized_answer || answer,
    candidate_source: 'learning_point_inventory',
    learning_point_schema_version: 1,
    learning_action: item.learning_action,
    learning_action_key: item.learning_action_key,
    ...
    teacher_note: `${duplicateHint ? `${duplicateHint} ` : ''}自动草稿卡：已进入需修复区，默认不会导出；请检查中文释义、例句和发音后再勾选。`,
