...
class CodexPluginPackageTests(unittest.TestCase):
    def test_repository_marketplace_exposes_the_passive_plugin(self) -> None:
        marketplace = json.loads(MARKETPLACE.read_text(encoding="utf-8"))

        self.assertEqual(marketplace["name"], "anki-study-agent-local")
        self.assertEqual(
            marketplace["interface"], {"displayName": "Anki Study Agent Local"}
        )
        self.assertEqual(
            marketplace["plugins"],
            [
    ...
    def test_passive_manifest_is_installable_without_fake_runtime_components(
        self,
    ) -> None:
        ...
        self.assertRegex(manifest["version"], r"^0\.1\.0(?:\+codex\.[a-z0-9-]+)?$")
    ...
    def test_skill_has_no_placeholders_and_routes_to_complete_contracts(self) -> None:
        ...
        for reference in references:
            ...
            reference_text = (SKILL / "references" / reference).read_text(
                encoding="utf-8"
            )
    ...
    def test_current_skill_workflow_names_only_real_public_tools(self) -> None:
        ...
        current_workflow = workflow.split("## Current public tool workflow", 1)[
            1
        ].split("## Planned but unavailable tools", 1)[0]
        named_tools = set(
    ...
    def test_skill_metadata_requires_explicit_skill_name_in_default_prompt(
        self,
    ) -> None:
