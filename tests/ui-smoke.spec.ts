...
test('desktop workflow shell supports simplified settings, URL mode, document mode, and generation', async ({ page }) => {
  ...
  await expect(page.getByLabel('生成审核概览').getByText('更多学习点')).toBeVisible()
  await expect(page.getByLabel('生成审核概览').getByText('重复 / 硬阻断')).toBeVisible()
  ...
  await page.getByRole('tab', { name: /素材配置/ }).click()
