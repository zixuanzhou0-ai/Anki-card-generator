...
- **Vertex Gemini 3.5 Flash**：模型配置里保留 Vertex `gemini-3.5-flash` 选项，并标注为“快速批量”；它适合低成本筛选，不建议作为最终高质量制卡首选。
- **APKG 导出路径修复**：普通项目不会再误触发 release 验收目录保护；URL 项目也会使用应用本地目录作为默认保存兜底，避免串到旧 `test_runs` 路径。
- **保底卡不可导出**：模型未完整返回时生成的保底卡会留在审核区显示为“需修复”，但不会再计入正式可导出卡，也不会混进 APKG。
- **TTS 前置硬门槛**：视频卡导出需要整句 TTS 和表达 TTS；TTS 关闭或未测试通过时，会在生成 APKG 前打开语音设置并阻止晚失败。
...
1. 安装或解压 Windows release 包；推荐使用 `v0.9.8-beta` 或更新版本。
